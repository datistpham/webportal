...
const AdminManageTeacherTeachObject= ()=> {
    return (
        <Admin>
...
function TeacherTeachObject() {
    ...
    const columns = [
      { field: 'id', headerName: 'ID', width: 90 },
      {
        ...
        width: 150,
        renderCell: (params)=> {
            return <>{params.row.first_name} {params.row.middle_name} {params.row.last_name}</>
        }
      },
      {
          field: 'course_name',
              ...
              return (
                  <div style={{display: "flex", justifyContent: "center", alignItems: "center", gap: 10}}>
                      ...
                      <DeleteOutlined onClick={async ()=> {
                          ...
                          .then(async value=> {
                              if(value=== "ok") {
                                const result= await delete_teacher_teach_subject(params.row?.id)
                                  if(result?.delete=== true) {
                                      swal("Notice", "Delete teacher teach subject successfully", "success")
...
export default AdminManageTeacherTeachObject